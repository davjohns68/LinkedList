...
public class LinkedList<E extends Comparable<E>> implements Iterable<E>{  
    ...
    All methods must work for empty or identical lists.
    */    

    // Exercise 2
    /**
     * Returns the minimum value from the list
     * @return Minimum value (from compareTo) in list
     */
    public E min() {
        ListNode<E> current = this.front.next;
        E min = current.data;         // Arbitrarily set to first item in the list
    ...
    // Exercise 9
    /**
     * Duplicates each value in the list.
     * Maintains original list order.
     */
    public void stutter() {
        ListNode<E> current = this.front.next;
        int index = 0;
    ...
    // Exercise 14
    /**
     * Removes all entries matching value from the list
     * @param value Entry to be removed
     */
    ...
    /**
     * Helper method that makes sure that all prev links are correct
     */
    private void ensureLinks() {
        ListNode<E> current = this.front.next;
        
        while (current.next != null) {
            // If the next node doesn't point to this one
            if (!(current.next.prev == current)) {
                current.next.prev = current;        // correct it
            }
            current = current.next;
        }
    }
